        ...
        public void onClick(View view) { // при нажатии кнопки во view записывается значение кнопки
            ...
            switch (view.getId()) { // switch (значение выбранного ID кнопки)
                ...
                case R.id.buttonStop: // нажатие кнопки "Стоп"
                    startTime = 0L;
                    timePause = 0L;
                    new CountDownTimer(timeInMilliseconds, 1) {

                        //Здесь обновляем текст счетчика обратного отсчета с каждой секундой
                        public void onTick(long millisUntilFinished) {
                            stopwatchOut.setText("" + millisUntilFinished / 86_400_000 + ":" + (millisUntilFinished / 3_600_000) % 24 + ":" + (millisUntilFinished / 60_000) % 60 + ":" + String.format("%02d", (millisUntilFinished / 1000) % 60) + ":" + String.format("%03d", millisUntilFinished % 1000));
                        }
                        //Задаем действия после завершения отсчета (высвечиваем надпись "Бабах!"):
                        public void onFinish() {
                            stopwatchOut.setText("0:0:0:00:000");
                        }
                    }
                            .start();
                    handler.removeCallbacks(updateTimerThread); // удаление из очереди данного потока
                    // загрузка сообщения о прекращении отсчёта времени
                    break;
            }
        }
    };
